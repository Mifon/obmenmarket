<?php
class cfOrderHandler extends msOrderHandler {
    ...
    public function changeOrderStatus($order_id, $status_id)
    {
        ...
        if ($order->save()) {
            ...
            if ($status->get('email_manager')) {
                ...
                if ( $prop['cf.from_city'] > 0 ) {
                    $mails = $this->modx->getObject('cfField', array(
                        'city_id' => $prop['cf.from_city'],
                        'placeholder' => 'ms2_email_manager'
                    ));
                }
                $mails = $mails ? $mails->get('value') : $this->modx->getOption('ms2_email_manager', null, $this->modx->getOption('emailsender'));
                $emails = array_map('trim', explode(',', $mails));
                if (!empty($subject)) {
