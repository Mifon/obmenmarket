...
class reMessages
{
    ...
    function getDialog($identifier)
    {
        ...
        if ($thread['name'] == 'personal') {
            $notify_identifier = 'id' . $this->modx->user->id;
        } else {
            $notify_identifier = 'group' . $thread['id'];
        }
    ...
    function findThread($identifier)
    {
        ...
        switch ($prefix) {
            case 'id':
                ...
                $q->groupby('rmThread.id');
                $q->where(['rmThread.name' => 'personal', 'CurrentUserMember.id:IS NOT' => NULL, 'FriendMember.id:IS NOT' => NULL]);
                if (!$q->prepare() || !$q->stmt->execute()) {
            ...
            case 'group':
                ...
                $access = false;
                foreach ($threadObj->Members as $member) {
                    if ($member->user == $this->modx->user->id) {
                        $access = true;
                        break;
                    }
                }
