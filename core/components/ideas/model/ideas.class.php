...
class ideas
{
    ...
    function add_idea($idea_type = 0, $idea_title = '', $idea_description = '', $context_key = 'web')
    {
        if($idea_type == 0){
            $data = [];
        ...
        if(!empty($context_key)){
            $context_key = $this->modx_tags($context_key);

            $context = $this->modx->getObject('modContext', array('key' => $context_key)) ;
            if(!$context){
                $context_key = 'web';
            }
        }
