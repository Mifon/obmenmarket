cityFields.grid.Cities = function(config) {
	...
	Ext.applyIf(config,{
		...
		,autosave: true
		,fields: ['id','key','name','active','isdefault']
		,autoHeight: true
		,paging: true
		,pageSize: 20
		,remoteSort: true
...
Ext.extend(cityFields.grid.Cities,MODx.grid.Grid,{
	...
	,getColumns: function() {
		all = {
			...
			,isdefault: {sortable: false, width: 40, renderer: cityFields.utils.rendererBoolean}
		};
		var columns = [this.sm];
...
cityFields.window.CreateCity = function(config) {
	...
	Ext.applyIf(config,{
		...
		,fields: [
			...
			,{xtype: 'textfield',fieldLabel: _('cityfields_cities_name'),name: 'name',id: 'cityfields-'+this.ident+'-name',anchor: '99%'}
			,{
				layout: 'column',
				items: [{
					columnWidth: .5,
					...
					layout: 'form',
					items: [
						{xtype: 'checkbox',fieldLabel: _('cityfields_cities_isdefault'),name: 'isdefault',id: 'cityfields-'+this.ident+'-isdefault',anchor: '10%'}
					]
				}]
			}
		]
